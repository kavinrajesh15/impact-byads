...
def run_pipeline(full_refresh: bool = True) -> int:
    """
    Main entry point:
      1. Fetch all ads
...
def main():
    parser = argparse.ArgumentParser(description="Impact.com Ad Performance → Google Sheets sync (1 Year Fill)")
    parser.add_argument("--incremental", action="store_true", help="Run incremental sync instead of full 1-year fill (not recommended for this script)")
    args = parser.parse_args()

    # Default to True for this specific 1-year sync script
    is_full = not args.incremental

    try:
        rows = run_pipeline(full_refresh=is_full)
